...
export default class OrderLines {
    ...
    public async GetOrderLineById(req: Request, res: Response) {
        try {
            if ((parseInt(req.params.ID_ORDER_LINE) < 0)) {
                res.status(400).send({ message: "Format de L'id est incorrect" });
            }
            else {
                await this._repository.GetOrderLineById(req.params.ID_ORDER_LINE)
    ...
    public async GetOrderLinesByOrderHeaderId(req: Request, res: Response) {
        ...
        try {
            await this._repository.GetOrderLinesByOrderHeaderId(req.params.ID_ORDER_HEADER)
                .then((data: OrderLine[]) => {
                    if (data) {
                        res.status(200).send(data);
                    } else {
    ...
    public async GetOrderTotalByOrderHeaderId(req: Request, res: Response) {
        await this.GetTotalSumOfProducts(req.params.ID_ORDER_HEADER).then((data) => {
    ...
    public async PutOrderLine(req: Request, res: Response) {
        const idOrderLine = req.params.ID_ORDER_LINE;
        if (!idOrderLine) {
